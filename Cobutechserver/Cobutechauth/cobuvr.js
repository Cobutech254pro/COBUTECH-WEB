...
const handleResendVerificationCode = (app, jwtSecret) => { // <--- ADDED jwtSecret here
    app.post('/api/auth/resend-verification-code', async (req, res) => {
        ...
        try {
            ...
            // If already verified, don't resend a code for an unverified status
            if (user.is_verified) {
            ...
            const newExpiryTime = new Date(Date.now() + 120000); // Code valid for 2 minutes
